...
class WallpaperMusicView extends GetView<WallpaperMusicController> {  
    ...
    return Padding(  
      ...
      child: Column(  
        ...
        children: [  
          ...
          const SizedBox(height: 20),  
          Obx(() {  
            if (controller.isWallpaperLoading) {  
              return const Expanded(  
                child: Center(  
                  child: CircularProgressIndicator(),  
                ),  
            ...
            if (controller.isWallpaperError) {  
              return Expanded(  
                child: Center(  
                  child: Text(  
                    'Error: ${controller.errorMessage.value}',  
  ...
  Widget _buildWallpaperItem(String wallpaper) {  
    return GetBuilder<WallpaperMusicController>(  
      builder: (_) {  
        final isSelected = controller.isWallpaperSelected(wallpaper);  
    ...
    return Container(  
      decoration: BoxDecoration(  
        ...
        image: DecorationImage(  
          ...
          colorFilter: isSelected  
              ? ColorFilter.mode(  
                  Colors.deepPurple.withOpacity(0.3), BlendMode.srcATop)  
              : null,  
        ),  
      ),  
    );  
  ...
  Widget _buildVideoPlayer(String wallpaper, bool isSelected) {  
    final videoController = controller.getVideoController(wallpaper);  
    ...
    return Padding(  
      ...
      child: Column(  
        ...
        children: [  
          ...
          Expanded(  
            child: Obx(() {  
              if (controller.isMusicLoading) {  
                return const Center(  
                  child: CircularProgressIndicator(),  
                );  
              }  
              if (controller.isMusicError) {  
                return Center(  
                  child: Text(  
                    'Error: ${controller.errorMessage.value}',  
                    style: const TextStyle(color: Colors.red),  
              ...
              return ListView.builder(  
                ...
                itemBuilder: (context, index) {  
                  final musicTrack = AssetPaths.musicTracks[index];  
                  return _buildMusicItem(musicTrack, index);  
                },  
              );  
            }),  
          ),  
          Obx(() => Slider(  
            value: controller.audioVolume.value,  
            onChanged: (value) => controller.setAudioVolume(value),  
            min: 0.0,  
            max: 1.0,  
  ...
  Widget _buildMusicItem(String musicTrack, int index) {  
    return GetBuilder<WallpaperMusicController>(  
      builder: (_) {  
        final isSelected = controller.isMusicSelected(musicTrack);  
